    ...
    for c1 in range(col):
        for c2 in range(c1, col):
            ...
            for r in range(row):
                ...
                # see below why these both are same
                # prefixSum[summ] = prefixSum.get(summ, 0) + 1
                if res == k: return k
                insort(arr, summ)   # insort = insert(arr, bisect(summ))    # O(n) time
    return res
...
# note that time complexity of insertion in SortedSet is O(logn)
# time complexity of insertion in list/array is O(n)


'''
Why the following two code are same

for item in dict:
    if prefixSum - target <= item:
        res = max(res, prefixSum - item)

And

idx = bisect_left(arr, prefixSum - target)
res = max(res, prefixSum - arr[idx])

here, once the prefixSum is calculated, it is fixed for this iteration
target is also fixed
hence, in the condition (prefixSum - target <= item), we need such item that satisfies this condition

in the line res = max(res, prefixSum - item), we need item's value to be minimum so that prefixSum - item becomes maximum
hence, we need an item which is minimum in value while satisfying the condition prefixSum - target <= item

if you look closely, bisect_left and arr[idx] are satisfying both of these conditions
'''